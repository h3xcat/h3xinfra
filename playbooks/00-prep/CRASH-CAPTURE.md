...
mounted and have exactly one matching, required writable `/etc/fstab` entry.
Optional, automatic, bind, and non-fstab mounts are unsupported. Filesystem
...
Only independently available local ext4/XFS on plain SATA, SAS, or PCIe NVMe
...
arguments are not replaced. The first subsequent normal boot generates the
minimal capture initramfs if needed and attempts to load the capture kernel.

Staging explicitly restores Noble's capture-only boot arguments and managed
kernel/initrd paths; custom capture command lines and paths are not retained.
...
inhibition, absence of reboot/load commands, and restrictive dump settings.
